...
const findButtonsDeal = (data,url)=>{
    data&&data.forEach((ele)=>{
        if(ele.resourceLink==url){
            resultButtons = deepClone(ele.buttons);
        } else if(ele.children){
            findButtonsDeal(ele.children,url);
        }
    })
...
const resourseDeal = (data)=>{
    data.forEach((ele,index)=>{
        ...
        ele.icon = ele.resourceIcon || 'appstore';
...
//折扣tab 卡类型数据结构处理
const dealDisCountTypeFun=(data)=>{
    data.forEach((ele)=>{
        ele.label = ele.typeName;
        ele.value = ele.isLeaf?ele.ecardType:ele.typeName;//第四级value为ecardType字段
        ele.children = ele.next;
        ele.children&&ele.children.length&&dealDisCountTypeFun(ele.children);
    })
    return data;
}
...
//获取左侧菜单栏的所有url
let resourceUrlList = []
const getResourceUrlList =(data)=>{
    data.forEach((ele,index)=>{
        if(ele.children){
            getResourceUrlList(ele.children);
        } else {
            resourceUrlList.push(ele.url);
        }
    })
    return resourceUrlList;
}

const setResourceUrlList =(data)=>{
    resourceUrlList = data;
}
