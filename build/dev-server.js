...
app.use(config[ENV]['uploadExcelFilter'], proxyMiddleware({
    target: config[ENV]['targetUploadUrl'],
    changeOrigin: true
}))
app.use('/file', proxyMiddleware({
    target: config[ENV]['targetUrl'],
    changeOrigin: true
}))
app.use('/cms-admin', proxyMiddleware({
    target: config[ENV]['targetContentUrl'],
    changeOrigin: true
}))

app.use('/dictionary', proxyMiddleware({
    target: config[ENV]['targetUrl'],
    changeOrigin: true
}))

app.use('/activityEntity', proxyMiddleware({
    target: config[ENV]['targetUrl'],
    changeOrigin: true
}))

app.use('/activityCard', proxyMiddleware({
    target: config[ENV]['targetUrl'],
    changeOrigin: true
}))

app.use('/process', proxyMiddleware({
    target: config[ENV]['targetUrl'],
    changeOrigin: true
}))
